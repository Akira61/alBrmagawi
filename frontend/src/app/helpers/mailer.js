...
export async function resetPassword(email, id, role) {
  try {
    // get user from email
    const findUser = await userExists(email, 'email');
    if(findUser.length ==0){
      return {success:false, err_message:'user doesn\'t exists'}
    }

    const token = await bcrypt.hash(findUser[0].id.toString(), 10);
    // update user token based on role
    let user;
    if (findUser[0].role === "student") {
      const updateToken = await prisma.users.update({
        ...
        data: {
          ...
          forgotPasswordTokenExpire: (Date.now() + 3600000).toString(),
        },
      });
      user = updateToken;
    ...
    const mailOptions = {
      ...
      html: `<h1>hi ${
        ...
        process.env.domain
      }/resetPasswordPage?token=${token}'>here</a> to reset your password</p>`,
    };
    const mail = await transport.sendMail(mailOptions);
    console.log(mail);
    return {data : mail, success:true};
  } catch (error) {
    throw new Error(error.message);
  }
}
