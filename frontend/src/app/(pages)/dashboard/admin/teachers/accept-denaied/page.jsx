...
export default function AllTeachers() {
  ...
  async function fetchData() {
    try {
      const { data } = await axios.get(
        "/api/dashboards/admin/teachers/accept-denaied"
      );
      if (data.success) {
  ...
  async function accept(userId) {
    try {
      const { data } = await axios.get(
        `/api/dashboards/admin/teachers/accept-denaied/accept/${userId}`
      );
      if (data.success) {
  ...
  //denaied teacheer
  async function denaied(userId) {
    try {
      const { data } = await axios.get(
        `/api/dashboards/admin/teachers/accept-denaied/denaied/${userId}`
      );
      if (data.success) {
        window.location.reload();
      }
        ...
        <div className="sidebar-settings h-screen py-28 flex-1 p-7">
          <>
            ...
            {teachers.length > 0 ? (
                      ...
                      <tr key={i} className="bg-white border-b font-semibold dark:bg-gray-800 dark:border-gray-700 ">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-between">
                            <a
                              onClick={() => accept(teacher.id)}
                              href={`#`}
                              title="Accept"
                              className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                            >
                              <FontAwesomeIcon
                                className="bg-green-400 text-gray-800 text-sm p-2 rounded-md"
                                icon={faCheck}
                              />{" "}
                            </a>
                            <a
                              onClick={() => denaied(teacher.id)}
                              href={"#"}
                              title="Denaied"
              ...
              <h1 className="text-center">no teacher found</h1>
            )}
          </>
        </div>
      </div>
