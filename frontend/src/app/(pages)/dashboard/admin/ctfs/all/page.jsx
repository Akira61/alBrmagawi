...
export default function AllAdminCTFs() {
  ...
  async function fetchData() {
    try {
      const { data } = await axios.get("/api/ctfs/adminAllctf");
      if (data.success) {
        setData(data.data);
      }
      if (!data.success) {
          ...
          <>
            ...
            <div class="flex items-center pb-4">
              <div className="relative">
                <Link href={`/dashboard/admin/ctfs/create`}>
                  <button
                ...
                <tbody>
                  {data.map((machine, i) => (
                    <tr key={i} className="bg-white border-b font-semibold dark:bg-gray-800 dark:border-gray-700 ">
                      ...
                      <td className="px-6 py-4">{machine.catigory}</td>
                      <td className="px-6 py-4">{machine.rating}</td>
                      ...
                      <td className="px-6 py-4">{machine.flag}</td>
