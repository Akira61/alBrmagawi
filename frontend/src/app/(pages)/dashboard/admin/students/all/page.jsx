...
export default function AllTeachers() {
  ...
  async function fetchData() {
    try {
      const { data } = await axios.get("/api/dashboards/admin/students/all");
      if (data.success) {
  ...
  async function deleteStudent(userId) {
    try {
      const { data } = await axios.get(
        `/api/dashboards/admin/students/delete/${userId}`
      );
      if (data.success) {
          ...
          <>
            ...
            <div class="flex items-center justify-end pb-4">
              <div className="relative">
                <a href={configURL.dashboard.admin.student.add}>
                  <button
                ...
                <tbody>
                  {students.map((student, i) => (
                    <tr key={i} className="bg-white border-b font-semibold dark:bg-gray-800 dark:border-gray-700 ">
                      ...
                      <td className="px-6 py-4">{student.phone_number?student.phone_number:'Null'}</td>
                      <td className="px-6 py-4">{student.email}</td>
                      <td className="px-6 py-4">{student.verified?"True":"False"}</td>
                      <td className="px-6 py-4">{student.joining_date}</td>
