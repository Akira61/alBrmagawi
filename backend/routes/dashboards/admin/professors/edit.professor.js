...
router.get('/dashboard/edit-professor', async(req, res) => {
    ...
    console.log(user);
    res.render('./dashboards/Professors/edit-professor.ejs', {data : user});
})



router.post('/dashboard/edit-professor', async (req, res) => {
    ...
    || !req.body.email || !req.body.gender || !req.body.Designation || 
    ...
    number,gender,Designation, department,
    birth, education, joining_date} = req.body;
    ...
    phone_number=?, gender=?, designation=?,
    department=?, birth_day =?, education =?, joining_date =?
    WHERE user_id=?;
    `,[firstName,lastName,email,
    number,gender,Designation,
    department,birth,education,
    joining_date, req.query.user_id], (err, result) => {
        if(err) throw err;
