...
router.get("/auth/forgot-password", (req, res) => {
  ...
  res.sendFile(
    path.join(__dirname + "../../../views/auth/forgot-password.html")
  );
});

router.post("/auth/forgot-password", async (req, res) => {
  if (!req.body.email)
    return res.json({ success: false, err_message: "no email sent" });
  ...
  if (user.length == 0)
    return res.json({
      success: false,
      err_message:
        'email not found. Please try to <a href="/auth/register">sign up</a>',
    });
  ...
  host = req.get("host");
  link =
    "http://" + req.get("host") + "/auth/forgot-password/reset-pass?id=" + rand;
  ...
  smtpTransport.sendMail(mailOptions, function (error, response) {
    if (error) {
      ...
      res.json({
        success: false,
        err_message: "couldn't send the link through email. please try later",
      });
    } else {
...
router.get("/auth/forgot-password/reset-pass", (req, res) => {
  if (req.protocol + "://" + req.get("host") == "http://" + host) {
    ...
    if (req.query.id == rand) {
      ...
      return res.sendFile(
        path.join(__dirname + "../../../views/auth/reset-password.html")
      );
    }
